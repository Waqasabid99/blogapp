...
const SkeletonRow = ({ columns }) => (
    <tr style={{ borderBottom: "1px solid var(--border-light)" }}>
        {columns.map((_, ci) => (
            <td key={ci} style={{ padding: "15px 18px" }}>
                <SkeletonCell width={ci === 0 ? "55%" : ci % 3 === 0 ? "45%" : "70%"} />
            </td>
        ))}
    </tr>
);

const SkeletonCard = ({ columns }) => (
    <div style={{ padding: "14px 16px", borderBottom: "1px solid var(--border-light)" }}>
        {columns.map((col, ci) => (
            <div key={ci} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "5px 0" }}>
                ...
                <SkeletonCell width={ci % 2 === 0 ? "35%" : "45%"} />
            </div>
        ))}
    </div>
);
        ...
        <>
            ...
            <div className="w-full rounded-lg overflow-x-auto" style={{ border: "1px solid var(--border-light)", background: "var(--bg-primary)", boxShadow: "var(--shadow-sm)" }}>
                ...
                <table className="w-full text-left text-sm hidden sm:table" style={{ borderCollapse: "collapse" }}>
                    ...
                    <tbody>
                        {isLoading ? (
                            skeletonRowsArray.map((_, ri) => (
                                <SkeletonRow key={ri} columns={columns} />
                            ))
                ...
                <div className="sm:hidden">
                    {isLoading ? (
                        skeletonRowsArray.map((_, ri) => (
                            <SkeletonCard key={ri} columns={columns} />
                        ))
                    ) : pageData.length > 0 ? pageData.map((row, ri) => (
                    ...
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "10px", padding: "12px 18px", borderTop: "1px solid var(--border-light)" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: "16px", flexWrap: "wrap" }}>
                            <div style={{ display: "flex", alignItems: "center", gap: "8px", fontSize: "13px", color: "var(--text-muted)" }}>
                                Rows per page:
                                {isLoading
                                    ? <div style={{ width: "52px", height: "32px", borderRadius: "6px", background: "linear-gradient(90deg, var(--bg-secondary) 25%, var(--bg-tertiary) 50%, var(--bg-secondary) 75%)", backgroundSize: "600px 100%", animation: "shimmer 1.4s infinite linear" }} />
                            ...
                            {isLoading
                                ? <div style={{ width: "70px", height: "13px", borderRadius: "4px", background: "linear-gradient(90deg, var(--bg-secondary) 25%, var(--bg-tertiary) 50%, var(--bg-secondary) 75%)", backgroundSize: "600px 100%", animation: "shimmer 1.4s infinite linear" }} />
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: "4px" }}>
                            {isLoading
                                ? Array.from({ length: 5 }).map((_, i) => (
                                    <div key={i} style={{ width: "32px", height: "32px", borderRadius: "6px", background: "linear-gradient(90deg, var(--bg-secondary) 25%, var(--bg-tertiary) 50%, var(--bg-secondary) 75%)", backgroundSize: "600px 100%", animation: "shimmer 1.4s infinite linear" }} />
                                ))
                                : <>
